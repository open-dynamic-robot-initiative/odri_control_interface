...
BOOST_PYTHON_MODULE(libodri_control_interface_pywrap)
{
    ...
    class_<IMU>("IMU", init<std::shared_ptr<MasterBoardInterface>>())
        ...
        .add_property("robot_interface", make_function(&IMU::GetMasterBoardInterface,
    ...
    class_<Robot>("Robot", init<std::shared_ptr<MasterBoardInterface>,
                  std::shared_ptr<JointModules>, std::shared_ptr<IMU>>())
        .def("init", &Robot::Init)
